...
export function analyzeResumeTextClient(
  ...
  if (futureDateMatch) {
    verificationAlerts.push({
      ...
      severity: 'high',
      confidence: 0.95,
      description: `Future work date anomaly detected (${futureDateMatch[1]}).`,
      detectedText: futureDateMatch[1],
      impact: 'Timeline chronological conflict.'
    });
  }

  // Check for keyword stuffing / invisible text patterns
  const microTextMatch = cleanText.match(/([a-zA-Z\s]{40,})\s*\1/i);
  if (microTextMatch) {
    verificationAlerts.push({
      id: `fraud_keyword_stuffing_${Date.now()}`,
      type: 'keyword_stuffing',
      severity: 'medium',
      confidence: 0.88,
      description: 'Repeated high-frequency keyword sequence detected in background.',
      detectedText: microTextMatch[1].slice(0, 50),
      impact: 'Possible ATS keyword manipulation.'
    });
  }
  ...
  const verificationStatus = verificationAlerts.length > 0 ? 'flagged' : 'verified';
  ...
  return {
    ...
    location,
    title,
    summary: `${title} with ${experienceYears}+ years of verified engineering experience. Demonstrated strength in ${skillsArray.slice(0, 4).join(', ')}.`,
    experienceYears,
    finalScore,
    semanticScore,
    keywordScore,
